  ...
  (let* ((enemy (trocar-jogador jogador)))
    (labels ((maximize (sucessores alpha beta profundidade adversario)
             ...
             (iter (no profundidade
               ...
               (let ((novos-sucessores (sucessores-alphabeta no jogador))
                     (solucao (estado-solucao (no-tabuleiro no))))
                 (cond ((null novos-sucessores) 0)
                       ((and (not (null solucao)) (= solucao (trocar-jogador adversario))) 9999)
                       ...
                       (t (minimize novos-sucessores alpha beta profundidade (trocar-jogador jogador)))))))
      (iter no 0))))